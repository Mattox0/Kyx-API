...
import { MostLikelyTo } from '../entities/most-likely-to.entity.js';
import { MostLikelyToTranslation } from '../entities/most-likely-to-translation.entity.js';
...
import { ImportMostLikelyToItemDto } from '../dto/import-most-likely-to.dto.js';
import { Gender } from '../../../types/enums/Gender.js';
import { shuffle } from '../../common/utils/shuffle.js';
import { DEFAULT_LOCALE } from '../../config/languages.js';
import { FlatMode, FlatMostLikelyTo } from '../../../types/ws/FlatQuestion.js';
import { UserSoloItemDto } from '../dto/create-party-most-likely-to.dto.js';
...
function toTranslationsMap(translations: MostLikelyToTranslation[]): Record<string, { question: string }> {
...
@Injectable()
export class MostLikelyToService {
  ...
  async findAll(
    ...
    const qb = this.dataSource
      .createQueryBuilder()
      .select('mlt')
      .from(MostLikelyTo, 'mlt')
      .leftJoinAndSelect('mlt.mode', 'mode')
      .leftJoinAndSelect('mode.translations', 'modeTranslation')
      .leftJoinAndSelect('mlt.translations', 'translation');
    ...
    if (search) {
      qb.andWhere((qb2) => {
        const sub = qb2
          .subQuery()
          .select('1')
          .from(MostLikelyToTranslation, 'searchTrans')
          .where('searchTrans.mostLikelyTo = mlt.id')
          .andWhere('searchTrans.question ILIKE :search')
          .getQuery();
        return `(EXISTS ${sub} OR CAST(mlt.id AS TEXT) ILIKE :search)`;
      });
    ...
    if (locale && locale_status) {
      ...
      qb.andWhere((qb2) => {
        const sub = qb2
          .subQuery()
          .select('1')
          .from(MostLikelyToTranslation, 'filterTrans')
          .where('filterTrans.mostLikelyTo = mlt.id')
  ...
  async create(dto: CreateMostLikelyToDto) {
    try {
      ...
      const translationsToInsert = Object.entries(dto.translations)
        .filter(([, val]) => val != null)
        .map(([locale, val]) => ({ mostLikelyTo: { id }, locale, question: val.question }));

      if (translationsToInsert.length > 0) {
        await this.dataSource
          .createQueryBuilder()
          .insert()
          .into(MostLikelyToTranslation)
  ...
  async update(id: string, dto: UpdateMostLikelyToDto) {
    try {
      const updateData: Partial<MostLikelyTo> = {};
          ...
          if (isEmpty) {
            if (locale === DEFAULT_LOCALE) {
              ...
              await this.dataSource
                .createQueryBuilder()
                .delete()
                .from(MostLikelyToTranslation)
                .where('"mostLikelyToId" = :id AND locale = :locale', { id, locale })
                .execute();
            }
          } else {
            await this.dataSource.getRepository(MostLikelyToTranslation).upsert(
              [{ mostLikelyTo: { id }, locale, question: (val!.question ?? '').trim() }],
              { conflictPaths: ['mostLikelyTo', 'locale'], skipUpdateIfNoValuesChanged: true },
            );
          }
        }
      }
  ...
  async getTranslationStats() {
    const total = await this.dataSource.getRepository(MostLikelyTo).count();
    const rows = await this.dataSource
      .createQueryBuilder(MostLikelyToTranslation, 't')
  ...
  async bulkCreate(items: ImportMostLikelyToItemDto[]): Promise<{ created: number; skipped: number; errors: string[] }> {
    let created = 0;
    let skipped = 0;
    const errors: string[] = [];
  ...
  async createPartySolo(
    dto: { users: UserSoloItemDto[]; modes: string[]; customQuestions?: CustomQuestionDto[] },
    locale: string = DEFAULT_LOCALE,
  ): Promise<{ question: FlatMostLikelyTo; questionType: 'most-likely-to'; userTarget: null; userMentioned: UserSoloItemDto | null }[]> {
    ...
    const customCount = (dto.customQuestions ?? []).filter((cq) => cq.type === 'most-likely-to').length;
    ...
    const randomIds = dbLimit === 0
      ...
      : await this.dataSource
          ...
          .select('mlt.id', 'id')
          .from(MostLikelyTo, 'mlt')
          .where('mlt.modeId IN (:...modeIds)', { modeIds: dto.modes })
          .andWhere('(mlt.mentionedUserGender IS NULL OR mlt.mentionedUserGender IN (:...allowedMentionedGenders))', { allowedMentionedGenders })
          .orderBy('RANDOM()')
          .limit(dbLimit)
    ...
    const questions = randomIds.length === 0
      ...
      : await this.dataSource
          ...
          .leftJoinAndSelect('mode.translations', 'modeTranslation')
          .leftJoinAndSelect('mlt.translations', 'translation')
          .where('mlt.id IN (:...ids)', { ids: randomIds.map((r) => r.id) })
          .getMany();
    ...
    const mapped = questions
      .map((question) => {
        ...
        const flatQuestion: FlatMostLikelyTo = {
          id: question.id,
        ...
        const hasUserPlaceholder = translation.question.includes('{user}');
        const genderToUse = question.mentionedUserGender ?? (hasUserPlaceholder ? Gender.ALL : null);

        return {
          question: flatQuestion,
          questionType: 'most-likely-to' as const,
          userTarget: null,
    ...
    const customMapped = (dto.customQuestions ?? [])
      .filter((cq) => cq.type === 'most-likely-to')
      .map((cq) => {
        const fakeQuestion: FlatMostLikelyTo = {
        ...
        return { question: fakeQuestion, questionType: 'most-likely-to' as const, userTarget: null, userMentioned: null };
      });
